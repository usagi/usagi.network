// Centralized content source configuration
export const SOURCES = {
 youtube: {
  // Prefer channelId if you have it; otherwise we'll use the handle only for links.
  channelId: null, // e.g., 'UCxxxxxxxxxxxxxxxxxx'
  // If you know uploads playlist id, set it to auto-updating official playlist embed:
  // uploadsPlaylistId = 'UU' + channelId.slice(2)
  uploadsPlaylistId: null,
  handle: 'usagi.network', // YouTube handle without leading '@'
  limit: 12,
  useOfficialPlaylistEmbed: false,
 },
 twitch: {
  channel: 'usaginetwork',
  clipsLimit: 12,
  vodsLimit: 12,
  useOfficialEmbeds: false,
 },
 soundcloud: {
  profileUrl: 'https://soundcloud.com/usagi-network',
  tracksUrl: 'https://soundcloud.com/usagi-network/tracks',
  limit: 50,
  // Optional worker base only for SoundCloud API (empty disables worker fetch for music).
  apiBase: '',
  // Static JSON endpoint for auto-refreshed tracks (auto-data branch via GitHub raw).
  dataUrl: 'https://raw.githubusercontent.com/usagi/usagi.network/auto-data/assets/data/soundcloud-tracks.json',
 },
 data: {
  // Auto-refreshed stream JSONs live on auto-data and are consumed directly.
  streamBaseUrl: 'https://raw.githubusercontent.com/usagi/usagi.network/auto-data/assets/data/stream',
 },
 api: {
  // Set your Cloudflare Worker base URL (no trailing slash), e.g. 'https://your-worker.workers.dev'
  base: '',
 }
};
