...
export async function mount()
{
 ...
 // If already initialized once, skip reloading
...
function renderCards(items, grid)
{
 ...
 if (list.length === 0)
 {
  ...
  msg.className = 'empty';
  msg.textContent = 'データの取得に失敗したようです。しばらくしてから再度お試しください。';
...
function openDetailEmbed(item)
{
 ...
 // Auto-pause BGM when a video is opened
...
function setupTwitchOfficialEmbeds()
{
 ...
 const chat = document.getElementById('twitch-chat');
 if (!liveWrap || !player || !chat) return;
 // Pause BGM as the official player will autoplay
 import('#utils/bgm.js').then(mod => mod.pauseIfPlaying?.()).catch(() => { });
...
function resolveApiBase()
{
 ...
 // Default: use configured remote base to avoid local CORS unless explicitly requested
