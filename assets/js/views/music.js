...
async function initPlayer(onReady)
{
 ...
 const profileUrl = SOURCES.soundcloud?.profileUrl || 'https://soundcloud.com/usagi-network';
 const tracksUrl = SOURCES.soundcloud?.tracksUrl || `${profileUrl}/tracks`;
 const embedOptions = {
  auto_play: false,
  visual: false,
  show_user: false,
 ...
 const finalizeReady = () =>
 {
  if (didFinalizeReady) return;
  didFinalizeReady = true;
 ...
 function renderList()
 {
  ...
  if (!tracks.length)
  {
   ...
   li.className = 'scp__track';
   li.innerHTML = `
				<div class="scp__idx">-</div>
				<div class="scp__tmeta">
					<div class="scp__t-title">Track list is loading or unavailable.</div>
					<div class="scp__t-artist">Open SoundCloud profile to play directly: <a class="scp__t-link" href="${profileUrl}" target="_blank" rel="noopener">${profileUrl}</a></div>
				</div>
				<div class="scp__dur">-</div>
			`;
 ...
 widget.bind(window.SC.Widget.Events.READY, () =>
 {
  ...
  if (useExternalList)
  {
   if (readyPass > 1) return; // ignore READY from subsequent widget.load calls
   widget.getSounds((list) =>
   {
    const widgetTracks = Array.isArray(list) ? list : [];
    // If widget can enumerate more tracks than worker/local JSON, prefer widget list.
    if (widgetTracks.length > tracks.length)
    {
     tracks = widgetTracks;
    ...
    currentIndex = 0;
    renderList();
    if (useExternalList && tracks.length > 0)
    {
     // Preload first track into the widget without forcing autoplay.
     loadByIndex(0, false);
    }
 ...
 // If SoundCloud widget never reports READY, do not keep the loading overlay forever.
 setTimeout(() =>
 {
  renderList();
  finalizeReady();
